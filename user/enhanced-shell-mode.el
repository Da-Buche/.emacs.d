;; =============================================================================================================
;; Script to tweak classic shell mode
;; 
;; Author: Aurelien Buchet
;; =============================================================================================================

;; ======================================================================
;; Function to wrap previous word with ${...}
;; ======================================================================
(require 'sh-script)

(defun sh-wrap-word nil
  "Wraps previous word with ${...}"
  (interactive) (left-word) (insert "${") (right-word) (insert "}"))

(add-hook 'sh-mode-hook (lambda nil (define-key sh-mode-map (kbd "C-}") 'sh-wrap-word)))

;; ======================================================================
;; Replacing default RET behavior
;; ======================================================================
;(require 'comint)

;; (defun comint-send-input (&optional no-newline artificial)
;;   "Modified so it does nothing except on last input line
...
;;          (pmark (if proc (process-mark proc) (user-error "Current buffer has no process"))))
;;     ;; Do something only if point is after process mark
;;     (when (>= (point) (marker-position pmark))
;;       (widen)
;;       (let* ((intxt (progn (if comint-eol-on-send
